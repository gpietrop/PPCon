...
def make_dim_scatter(a_list, variable):
    if variable == "NITRATE":
        list_scatter_dim = [150 if loss < 0.5 else 400 if 0.6 < loss < 0.75 else 800 for loss in a_list]
    if variable == "CHLA":
        list_scatter_dim = [150 if loss < 0.075 else 400 if 0.075 < loss < 0.1 else 800 for loss in a_list]
    if variable == "BBP700":
        list_scatter_dim = [150 if loss < 0.00025 else 400 if 0.00025 < loss < 0.0003 else 800 for loss in
                            a_list]
...
def plot_scatter_paper(variable, date_model, epoch_model, mode):
    ...
    if variable == "NITRATE":
        ...
        lg1 = ax.legend(legend_elements, ["RMSE<" + r"$0.6$", r"$0.6<$" + "RMSE" + r"$<0.7$", "RMSE" + r"$>0.7$"],
                        fontsize="10", title=f"RMSE [{un_meas}]", loc="lower right")
    if variable == "CHLA":
        un_meas = dict_unit_measure["CHLA"]
        lg1 = ax.legend(legend_elements, ["RMSE<" + r"$0.075$", r"$0.075<$" + "RMSE" + r"$<0.1$", "RMSE" + r"$>0.1$"],
                        fontsize="10", title=f"RMSE [{un_meas}]", loc="lower right")
    if variable == "BBP700":
        un_meas = dict_unit_measure["BBP700"]
        lg1 = ax.legend(legend_elements, ["RMSE<" + r"$2.5e^{-4}$", r"$2.5e^{-4}<$" + "RMSE" + r"$<3e^{-4}$",
                                          "RMSE" + r"$>3e^{-4}$"],
