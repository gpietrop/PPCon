...
def profile(variable, date_model, epoch_model, mode):
    ...
    dict_color = {'NWM': "red", 'SWM': "green", 'TIR': "blue", 'ION': "orange", 'LEV': "yellow"}
    ...
    for ax in axs.flat:
        ax.set(xlabel=variable, ylabel='depth')
    ...
    plt.legend(loc='center left', bbox_to_anchor=(1, 0.5), fontsize="3")

    plt.savefig(f"{path_analysis}profile_comparison.png")
...
def seasonal_and_geographic_bp(variable, date_model, epoch_model, mode):
    ...
    axs[0, 0].boxplot(list_loss[0])
    axs[0, 0].set_title(list(dict_season.keys())[0])

    axs[0, 1].boxplot(list_loss[1])
    axs[0, 1].set_title(list(dict_season.keys())[1])

    axs[1, 0].boxplot(list_loss[2])
    axs[1, 0].set_title(list(dict_season.keys())[2])

    axs[1, 1].boxplot(list_loss[3])
    axs[1, 1].set_title(list(dict_season.keys())[3])

    for ax in axs.flat:
        ax.set(xlabel=variable, ylabel='fitness')
        if variable == "CHLA":
